...
public record NoticeSummary(Long id, NoticeCategory category, String title, LocalDateTime createdAt) {

    public static NoticeSummary from(Notice notice) {
        return new NoticeSummary(notice.getId(), notice.getCategory(), notice.getTitle(), notice.getCreatedAt());
    }
}
